...
def count_old(cTime, echoing='on'):
    ...
    while detstats[1]<cTime*1000:
        ...
        detstats=readDetectorStats()
        if echoing=='on':
            print "\r{0} {1} {2} {3}".format(detstats[0], detstats[1], detstats[2], detstats[3]),
            sys.stdout.flush()
    return detstats
...
def rallh():
    ...
    for i in range(8):     #Triple axis angles
        if (i%4==0) & (i!=0):
            print
        print axisDictList[i][0].rjust(8)+'{0:7.3f}'.format(readang(axisDictList[i][1],echoing='off')),
    print 
...
def move(axis, targetPos, echoing="on", logging="on", wrend="on"):
    ...
    while isMoving(axis)==1:
        if echoing == "on":
            print axisDictRev[axis]+': {0:9.2f}'.format(readang(axis, echoing='off'))+'\r',
            sys.stdout.flush()
        sleep(0.5)
    if (echoing == "on"):
        if (wrend=="on"):
...
def printOffsets():
    ...
    for i in range(8):     #Triple axis angles
        if (i%4==0) & (i!=0):
            print
        print axisDictList[i][0].rjust(8)+'{0:7.3f}'.format(actSpect.axisOffsetDict[ axisDictList[i][0] ] ),
    print 
...
def printLimits():
    ...
    for i in range(8):     #Triple axis angles
        if (i%4==0) & (i!=0):
            print
        print axisDictList[i][0].rjust(8)+'{0:12}'.format(actSpect.axisLimitsDict[ axisDictList[i][0] ] ),
    print 
